...
export class GlobalScheduleComponent {
  ...
  getDisplayedGroup(): string[] {
    ...
    if (this.selectedGroup === 'ING1_INFO') {
      this.displayedGroup.push('ING1_INFO_TD1', 'ING1_INFO_TD1 || ING1_INFO_TD2', 'ING1_INFO');
    }
  ...
  getNiveaux(day: string): string[] {
    return Object.keys(this.schedule[day]);
  }

  getTimes(day: string, niveau: string): string[] {
    return Object.keys(this.schedule[day][niveau]);
  }

  getTotalRowSpan(day: string): number {
    return Object.keys(this.schedule[day])
      .map(niveau => this.getTotalRowSpanForNiveau(day, niveau))
      .reduce((sum, count) => sum + count, 0);
  }

  getTotalRowSpanForNiveau(day: string, niveau: string): number {
    return Object.keys(this.schedule[day][niveau])
      .map(time => this.schedule[day][niveau][time].length)
      .reduce((sum, count) => sum + count, 0);
  }
